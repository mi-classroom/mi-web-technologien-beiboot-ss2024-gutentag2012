...
export const FrameInputs = ({
	focussedImage,
	max,
}: { focussedImage: Signal<number>; max: number }) => {
	...
	return frames.map((frame, i) => (
		<field.SubFieldProvider
			...
			transformFromBinding={(v: string) => Number.parseInt(v)}
		>
			{(subField) => (
					...
					<Button
						...
						onClick={() => {
							...
							const middle = Math.max(
								currentValue + 1,
								Math.floor((frames[i + 1].data.peek() - currentValue) / 2) +
									currentValue,
							);
