...
export async function uploadVideoFile(video: File | null, additionalData: Array<[string, string]> | undefined, status: Signal<FileUploadState>) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", `${process.env.NEXT_PUBLIC_BACKEND_URL}/file-upload`, true)
    xhr.send(formData)
    status.value = {
      state: "progress",
      progress: 0
...
export async function deleteFolder(...paths: string[]) {
  return fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/file-upload/delete/${paths.join(encodeURIComponent("/"))}`, {method: "DELETE"})
}