...
export default async function RootLayout({
	...
	const memoryUsagePercent = memoryUsage.totalUsage / memoryUsage.maxUsage * 100
	const [progressIndicator, progressBackground] = memoryUsagePercent <= 70 ? ["bg-primary", "pg-primary/40"] : memoryUsagePercent >= 90 ? ["bg-destructive", "bg-destructive/40"] : ["bg-orange-700", "bg-orange-700/40"]
			...
			>
				<header className="bg-card w-full h-16 flex flex-row items-center justify-between px-4">
					...
					<div className="flex flex-col items-end gap-1">
						<Progress classNameIndicator={progressIndicator} className={`w-64 ${progressBackground}`} value={memoryUsagePercent} />
						<Tooltip>
							<TooltipTrigger asChild>
								<div className="text-xs text-nowrap flex flex-row gap-1">
									<span className="font-bold">Current memory usage:</span>
									<span>{memoryUsage.totalUsage.toFixed(2)} / {memoryUsage.maxUsage.toFixed(2)} GB</span>
									<InfoCircledIcon />
								</div>
							</TooltipTrigger>
							<TooltipContent className="max-w-52 bg-foreground text-background">
								The memory limit is a soft limit and will allow go over it, but once it is reached, no new projects, stacks or images can be created.
							</TooltipContent>
						</Tooltip>
					</div>
				</header>
